...
interface FindInventoryProps {
  searchBy: string;
}

const FindInventoryByIdFormContent: React.FC<FindInventoryProps> = ({
  searchBy,
}) => {
  ...
  const onFinish = async (values: { id?: number; name?: string }) => {
    ...
    if (searchBy === "id") {
      ...
      value = values.name;
      path = "/api/inventory/api/inventory/name/";
    }
    try {
      ...
      console.log(values);
      console.log(value);
      navigate(`/inventory/${data.id}/show`, {
...
export default FindInventoryByIdFormContent;
